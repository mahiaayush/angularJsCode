...
export class UpdateTitleComponent extends BaseComponent {
  ...
  NAME_PAD_STATUS_RESOURCES_LIST = 'PAD_STATUS_RESOURCES_LIST';
  ...
  padStatusDropdwonList: Array<any> = [];
  activeLink: any = null;
  isShow: boolean;
  isVisible: boolean;
  viewUpdateArray: any;
  ...
  getNonSearchModelParams(name: any): string | any {
    if (name === this.NAME_VIEW_TITLE) {
      const obj = {
        ...
        vendorId: '' || this.updateTitleModel.vendorId.value
      };
      return obj;
    }
  }
  ...
  handleLoadDataFromApi(name: string, data: any) {
    if (name === this.NAME_VIEW_TITLE) {
      this.viewUpdateArray = data.titleBean;
      ...
      const authorsList = this.viewUpdateArray.authors;
      const checkIsObj = Array.isArray(authorsList);
      if (checkIsObj === true) {
        this.authorsList = this.viewUpdateArray.authors;
      } else {
        this.authorsList = [this.viewUpdateArray.authors];
      }
      // this.updateTitleModel.name.value = [this.viewUpdateArray.authors];
      ...
      this.updateTitleModel.vendor.value = this.viewUpdateArray.contentVendor.isPrimary;
      const vendorList = data.vendors;
      const isArrayOrObj = Array.isArray(vendorList);
      if (isArrayOrObj === true) {
      ...
      this.hideLoader();
    } else if (name === this.NAME_PAD_STATUS_RESOURCES_LIST) {
      this.padStatusDropdwonList = data;
    } else if (name === this.NAME_UPDATE_TITLE) {
      this.hideLoader();
      this.state = true;
      this.message = 'Updated successfully.';
      ProjectUtils.html.showMsgForDuration('Update-Title');
    }
  }
  ...
  getServiceUrl(name): string {
    if (name === this.NAME_UPDATE_TITLE) {
      ...
      return UrlConstants.PAD_STATUS_RESOURCES_LIST;
    }
  }
  ...
  updateTitle() {
    this.showLoader();
    this.loadDataFromApi(this.NAME_UPDATE_TITLE);
  }
  ...
  padStatusList() {
    this.loadDataFromApi(this.NAME_PAD_STATUS_RESOURCES_LIST);
  }
