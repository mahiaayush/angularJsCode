...
export abstract class BaseComponent implements OnInit, OnDestroy, AfterViewInit {
      ...
      @Input() customReInit: Subject<any>;
      subcusTomReInit: Subscription;
      ...
      getSearchModel(name: string) {
            console.log("getSearchModel", name);
      }
      ...
      Sub_CustomReInit() {
            this.xtCustomReInit();
            if ((this.customReInit instanceof Subject) && (!ProjectUtils.isSubscribed(this.subcusTomReInit))) {
                  this.subcusTomReInit = this.customReInit.subscribe((data) => {
                        this.CustomReInitTask(data);
                  });
            }
      }
      xtCustomReInit() {
      ...
      ngOnDestroy() {
            ProjectUtils.unsubscribe(this.subcusTomReInit);
      ...
      loadDataFromApiNSetGrid(name: string, bindDataToGrid = false) {
            ...
            this.baseService.getDataFromAPI(url, params)
                  .subscribe((data) => {
                        ...
                        if (this.baseService.apiSessionCheck(data)) {
      ...
      loadDataFromApi(name: string): void {
            //  const paramsBody = this.getParamsBody(name);
            //  const url = this.getServiceUrl(name);
            // this.showLoader();
            const { url, params } = this.getQueryUrl(name);
            ...
            this.baseService.getDataFromAPI(url, params)
                  .subscribe((data) => {
                        ...
                        this.hideLoader();
                        if (this.baseService.apiSessionCheck(data)) {
                              this.handleLoadDataFromApi(name, data);
                        }
                  }, error => {
                        this.handleErrorLoadDataFromApi(name, error);
                  });
      ...
      getValueFromSearchModel(name: string): any {
            ...
            let value = null;
            keys.forEach((key) => {
                  value = searchModel[key].value;
                  retValue[key] = value === '' ? null : value;
            });
            return retValue;
      }
      ...
      getQueryParams(name: string) {
            ...
            if (keys.length !== 0) {
                  keys.forEach((key) => {
                        const val: BasicSearchModelI = searchModel[key];
                        retValue[val.apiKey] = val.value === '' ? null : val.value;
                  });
